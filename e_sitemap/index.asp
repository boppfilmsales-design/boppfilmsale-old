...
rs.open "select * from  yuzhiguo_e_other",conn,1,1
if rs.eof and rs.bof then
else
%>
<%
do while not rs.eof%>
<a href="../html_info/<%=rs("html_url")%>" target="_blank" ><%=rs("name")%></a> |
<%rs.movenext
loop
%>
<%
...
<%
do while not rs.eof%>
<a href="../html_products/<%=rs("html_url_en")%>" target="_blank" ><%=rs("e_cpmc")%> - <%=rs("cpbh")%></a> |
<%rs.movenext
loop
%>
<%
...
<h1>News Category</h1>
<%set rs=server.CreateObject("adodb.recordset")
rs.open "select * from  yuzhiguo_e_news_class",conn,1,1
if rs.eof and rs.bof then
else
%>
<%
do while not rs.eof%>
<div>●<a href="../e_news/?class_id=<%=rs("class_id")%>" target="_blank" ><%=rs("name")%></a></div>
<%rs.movenext
loop
%>
<%
...
<h1>News List</h1>
<%set rs=server.CreateObject("adodb.recordset")
rs.open "select * from  yuzhiguo_e_news",conn,1,1
if rs.eof and rs.bof then
else
%>
<%
do while not rs.eof%>
<a href="../html_news/<%=rs("html_url")%>" target="_blank" ><%=rs("name")%></a> |
<%rs.movenext
loop
%>
<%
