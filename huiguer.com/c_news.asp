...
<tr>
<td colspan="9" align=center class="a1">中文新闻管理</td>
</tr>
...
sql ="select * from yuzhiguo_news where name like '%"&keywords&"%' or content like '%"&keywords&"%' order by "&paixu&" "&xu&""
...
<td align="center"><a href="../c_news_show/?id=<%=rs("id")%>" target="_blank">查看</a></td>
<td align="center"><a href="c_html_news.asp?action=html_id&amp;id=<%=rs("id")%>">生成</a></td>
...
<tr>
<td colspan="3" align=center class="a1">添加中文新闻</td>
</tr>
<tr>
...
rs_class.open "select * from yuzhiguo_news_class  order by class_id asc" ,conn,1,1
...
<tr>
  ...
  <td colspan="2" class="a3"><input name="author" type="text" id="author" value="<%=yuzhiguo_name%>" size="20" maxlength="50" /></td>
</tr>
<tr>
...
If hits="" Then hits=0
set rs = server.CreateObject ("adodb.recordset")
'sql="select * from yuzhiguo_news Where name='"&name&"'"
sql="select * from yuzhiguo_news "
...
set rs = server.CreateObject ("adodb.recordset")
sql="select * from yuzhiguo_news where id="&id&""
rs.open sql,conn,1,1
host="c_news_show/?id="+Cstr(rs("id"))'前台动态地址
folder="../c_html_news/"'文件夹
...
<tr>
<td colspan="3" align=center class="a1">修改中文新闻</td>
</tr>
<tr>
<td width="20%" align="center" class="a3">新闻标题</td>
...
rs_del.open "select * from [yuzhiguo_news]  where id="&del_id,conn,1,1

sql="delete from [yuzhiguo_news] where id="&del_id
file_html="../c_html_news/"+rs_del("html_url")'定义要删除的文件
conn.execute sql
...
set rs=conn.Execute("select * from yuzhiguo_news where id="&pl_id)
conn.Execute("update yuzhiguo_news set name='"&name&"',sort="&sort&" where id="&pl_id)
next
