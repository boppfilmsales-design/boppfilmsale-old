...
filename="c_news_class.asp"'页面名称
id=trim(request("id"))
...
<tr>
<td colspan="7" align=center class="a1">中文新闻分类管理</td>
</tr>
...
sql ="select * from yuzhiguo_news_class where name like '%"&keywords&"%' or content like '%"&keywords&"%' order by "&paixu&" "&xu&""
...
<td align="center"><%=Mydb("Select Count([id]) From [yuzhiguo_news] where [class_id]="&rs("class_id")&"",1)(0)%></td>
...
<tr>
<td colspan="3" align=center class="a1">添加中文新闻分类</td>
</tr>
<tr>
...
If sort="" Then sort=1
set rs = server.CreateObject ("adodb.recordset")
'sql="select * from yuzhiguo_news_class Where name='"&name&"'"
sql="select * from yuzhiguo_news_class "
...
id=request("id")
set rs = server.CreateObject ("adodb.recordset")
sql="select * from yuzhiguo_news_class where class_id="& id &""
rs.open sql,conn,1,1
%>
<table width="99%" border="0"  align=center cellpadding="3" cellspacing="1" bgcolor="#FFFFFF" class="a2">
<form action="<%=filename%>?action=savedit" name="myform" method=post>
<tr>
<td colspan="3" align=center class="a1">修改中文新闻分类</td>
</tr>
<tr>
...
del_id=replace(request.form("id")(i),"'","")
	...
	sql="select * from yuzhiguo_news where class_id="&del_id&""
	rs_del.open sql,conn,1,1
	if rs_del.eof and rs_del.bof then 
	set rs=conn.execute("delete from yuzhiguo_news_class where class_id In(" & del_id & ")")
	set rs=conn.execute("delete from yuzhiguo_news where class_id In(" & del_id & ")")
	else 
...
set rs=conn.Execute("select * from yuzhiguo_news_class where class_id="&pl_id)
conn.Execute("update yuzhiguo_news_class set name='"&name&"',content='"&content&"',sort="&sort&" where class_id="&pl_id)
next
