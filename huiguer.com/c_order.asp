...
<tr>
<td colspan="8" align=center class="a1">中文订单管理</td>
</tr>
...
sql ="select * from yuzhiguo_order where name like '%"&keywords&"%' or content like '%"&keywords&"%' order by "&paixu&" "&xu&""
...
id=request("id")
set rs = server.CreateObject ("adodb.recordset")
sql="select * from yuzhiguo_order where id="& id &""
rs.open sql,conn,1,1
%>
...
<tr>
<td colspan="3" align=center class="a1">查看中文订单</td>
</tr>
<tr>
...
<tr>
  ...
  <td colspan="2" class="a3"><a href="http://www.yuzhiguo.com/tool/ip/?ip=<%=rs("ip")%>" target="_blank"><%=rs("ip")%>[点击查询IP位置]</a></td>
</tr>
<tr>
...
set rs=conn.execute("delete from yuzhiguo_order where id In(" & id & ")")
