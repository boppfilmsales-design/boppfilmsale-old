...
<tr>
<td colspan="8" align=center class="a1">英文单栏目管理</td>
</tr>
...
sql ="select * from yuzhiguo_e_other where name like '%"&keywords&"%' or content like '%"&keywords&"%' order by "&paixu&" "&xu&""
...
<td align="center"><input name="html_url" type="text" disabled="disabled" id="html_url" value="html_info/<%=rs("html_url")%>" size="50" maxlength="255" />
  <a href="e_menu.asp?lanmuname=<%=rs("name")%>&amp;lanmuurl=html_info/<%=rs("html_url")%>">加入英文导航</a></td>
<td align="center"><input name="sort" type="text" id="sort" value="<%=rs("sort")%>" size="4" maxlength="10" onKeyDown="myKeyDown()"></td>
<td align="center"><a href="../e_info/?id=<%=rs("id")%>" target="_blank">查看</a></td>
<td align="center"><a href="e_html_info.asp?action=html_id&amp;id=<%=rs("id")%>">生成</a></td>
...
<tr>
<td colspan="3" align=center class="a1">添加英文单栏目</td>
</tr>
<tr>
...
<tr>
  ...
  <td colspan="2" class="a3"><input name="html_url" type="text" id="html_url" size="60" maxlength="255" />
    .html （提醒：如不填写则以栏目名称命名，一经设置不可更改）</td>
</tr>
<tr>
...
If sort="" Then sort=0
set rs = server.CreateObject ("adodb.recordset")
'sql="select * from yuzhiguo_e_other Where name='"&name&"'"
sql="select * from yuzhiguo_e_other "
...
rs.open sql,conn,1,1
host="e_info/?id="+Cstr(rs("id"))'前台动态地址
folder="../html_info/"'文件夹
...
<tr>
<td colspan="3" align=center class="a1">修改英文单栏目</td>
</tr>
<tr>
<td width="20%" align="center" class="a3">栏目名称</td>
...
rs_del.open "select * from [yuzhiguo_e_other]  where id="&del_id,conn,1,1

sql="delete from [yuzhiguo_e_other] where id="&del_id
file_html="../html_info/"+rs_del("html_url")'定义要删除的文件
conn.execute sql

file_delete file_html

file_url=replace(file_html,"../","")'删除栏目同时删除导航链接
set rs=conn.execute("delete from yuzhiguo_e_menu where url='"&file_url&"'")
...
set rs=conn.Execute("select * from yuzhiguo_e_other where id="&pl_id)
conn.Execute("update yuzhiguo_e_other set name='"&name&"',sort="&sort&" where id="&pl_id)
next
