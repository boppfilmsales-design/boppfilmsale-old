...
<tr>
<td colspan="9" align=center class="a1">英文友情链接管理</td>
</tr>
...
sql ="select * from yuzhiguo_e_link where name like '%"&keywords&"%' or url like '%"&keywords&"%' order by "&paixu&" "&xu&""
...
<tr>
<td colspan="3" align=center class="a1">添加英文友情链接</td>
</tr>
<tr>
...
If sort="" Then sort=1
set rs = server.CreateObject ("adodb.recordset")
'sql="select * from yuzhiguo_e_link Where name='"&name&"'"
sql="select * from yuzhiguo_e_link "
...
id=request("id")
set rs = server.CreateObject ("adodb.recordset")
sql="select * from yuzhiguo_e_link where link_id="& id &""
rs.open sql,conn,1,1
%>
<table width="99%" border="0"  align=center cellpadding="3" cellspacing="1" bgcolor="#FFFFFF" class="a2">
<form action="<%=filename%>?action=savedit" name="myform" method=post>
<tr>
<td colspan="3" align=center class="a1">修改英文友情链接</td>
</tr>
<tr>
...
set rs=conn.execute("delete from yuzhiguo_e_link where link_id In(" & id & ")")
...
set rs=conn.Execute("select * from yuzhiguo_e_link where link_id="&pl_id)
conn.Execute("update yuzhiguo_e_link set name='"&name&"',url='"&url&"',class_id="&class_id&",show="&show&",sort="&sort&" where link_id="&pl_id)
next
