...
<tr>
<td colspan="7" align=center class="a1">英文导航管理</td>
</tr>
...
sql ="select * from yuzhiguo_e_menu where name like '%"&keywords&"%' or url like '%"&keywords&"%' order by "&paixu&" "&xu&""
...
<tr>
<td colspan="3" align=center class="a1">添加英文导航</td>
</tr>
<tr>
...
set rs = server.CreateObject ("adodb.recordset")
'sql="select * from yuzhiguo_e_menu Where name='"&name&"'"
sql="select * from yuzhiguo_e_menu "
...
id=request("id")
set rs = server.CreateObject ("adodb.recordset")
sql="select * from yuzhiguo_e_menu where id="& id &""
rs.open sql,conn,1,1
%>
<table width="99%" border="0"  align=center cellpadding="3" cellspacing="1" bgcolor="#FFFFFF" class="a2">
<form action="<%=filename%>?action=savedit" name="myform" method=post>
<tr>
<td colspan="3" align=center class="a1">修改英文导航</td>
</tr>
<tr>
...
set rs=conn.execute("delete from yuzhiguo_e_menu where id In(" & id & ")")
...
set rs=conn.Execute("select * from yuzhiguo_e_menu where id="&pl_id)
conn.Execute("update yuzhiguo_e_menu set name='"&name&"',url='"&url&"',show="&show&",sort="&sort&" where id="&pl_id)
next
