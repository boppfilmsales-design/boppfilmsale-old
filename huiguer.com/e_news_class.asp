...
filename="e_news_class.asp"'页面名称
id=trim(request("id"))
...
<option value="<%=filename%>?page=1&amp;pagesize=<%=pagesize%>&amp;xu=<%=xu%>&amp;paixu=class_id&amp;keywords=<%=keywords%>" <% if paixu="class_id" then %>selected<% end if %>>按【默认】排序</option>
<option value="<%=filename%>?page=1&amp;pagesize=<%=pagesize%>&amp;xu=<%=xu%>&amp;paixu=name&amp;keywords=<%=keywords%>" <% if paixu="name" then %>selected<% end if %>>按【分类名称】排序</option>
<option value="<%=filename%>?page=1&amp;pagesize=<%=pagesize%>&amp;xu=<%=xu%>&amp;paixu=content&amp;keywords=<%=keywords%>" <% if paixu="content" then %>selected<% end if %>>按【分类备注】排序</option>
...
<tr>
<td colspan="7" align=center class="a1">英文新闻分类管理</td>
</tr>
<tr  style="font-weight:bold;">
<td height="30" align="center" class="a1">选择</td>
<td align="center" class="a1">ID</td>
<td height="30" align="center" class="a1">分类名称</td>
<td align="center" class="a1">分类备注</td>
<td align="center" class="a1">新闻数量</td>
<td height="25" align="center" class="a1">排序</td>
<td height="25" align="center" class="a1">管理</td>
</tr>
<%
set rs=server.createobject("adodb.recordset")
sql ="select * from yuzhiguo_e_news_class where name like '%"&keywords&"%' or content like '%"&keywords&"%' order by "&paixu&" "&xu&""
...
<td align="center"><input name="content" type="text" id="content" value="<%=rs("content")%>" size="50" maxlength="255"></td>
<td align="center"><%=Mydb("Select Count([id]) From [yuzhiguo_e_news] where [class_id]="&rs("class_id")&"",1)(0)%></td>
...
<tr>
<td colspan="3" align=center class="a1">添加英文新闻分类</td>
</tr>
<tr>
<td width="20%" align="center" class="a3">分类名称</td>
<td width="80%" colspan="2" class="a3"><input name="name" type="text" id="name" size="50" maxlength="50"></td>
</tr>

<tr>
<td align="center" class="a3">分类备注</td>
<td colspan="2" class="a3"><input name="content" type="text" id="content" size="50" maxlength="255" /></td>
</tr>
<tr>
...
If sort="" Then sort=1
set rs = server.CreateObject ("adodb.recordset")
'sql="select * from yuzhiguo_e_news_class Where name='"&name&"'"
sql="select * from yuzhiguo_e_news_class "
...
id=request("id")
set rs = server.CreateObject ("adodb.recordset")
sql="select * from yuzhiguo_e_news_class where class_id="& id &""
rs.open sql,conn,1,1
%>
<table width="99%" border="0"  align=center cellpadding="3" cellspacing="1" bgcolor="#FFFFFF" class="a2">
<form action="<%=filename%>?action=savedit" name="myform" method=post>
<tr>
<td colspan="3" align=center class="a1">修改英文新闻分类</td>
</tr>
<tr>
<td width="20%" align="center" class="a3"><span class="a3">分类名称</span></td>
<td width="80%" colspan="2" class="a3"><input name="name" type="text" id="name" value="<%=rs("name")%>" size="50" maxlength="50"></td>
</tr>

<tr>
<td align="center" class="a3">分类备注</td>
<td colspan="2" class="a3"><input name="content" type="text" id="content" value="<%=rs("content")%>" size="50" maxlength="255" /></td>
</tr>
<tr>
<td align="center" class="a3">排序</td>
...
<tr>
<td width="20%" align="center" class="a3"><input name="id" type="hidden" id="id" value="<%=rs("class_id")%>">
...
del_id=replace(request.form("id")(i),"'","")
	...
	sql="select * from yuzhiguo_e_news where class_id="&del_id&""
	rs_del.open sql,conn,1,1
	if rs_del.eof and rs_del.bof then 
	set rs=conn.execute("delete from yuzhiguo_e_news_class where class_id In(" & del_id & ")")
	set rs=conn.execute("delete from yuzhiguo_e_news where class_id In(" & del_id & ")")
	else 
	Call Alert ("该分类下还有新闻，请先删除新闻，再删除分类!","-1")
	rs_del.close 
	set rs_del=nothing
	end if
next
...
Server.ScriptTimeout=99999999
Dim pl_id,name,content,sort
For i=1 to request.form("pl_id").count
pl_id=replace(request.form("pl_id")(i),"'","")
name=replace(request.form("name")(i),"'","")
content=replace(request.form("content")(i),"'","")
sort=replace(request.form("sort")(i),"'","")
If replace(request.form("sort")(i),"'","")="" Then sort=0
set rs=conn.Execute("select * from yuzhiguo_e_news_class where class_id="&pl_id)
conn.Execute("update yuzhiguo_e_news_class set name='"&name&"',content='"&content&"',sort="&sort&" where class_id="&pl_id)
next
