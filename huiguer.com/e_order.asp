...
<option value="<%=filename%>?page=1&amp;pagesize=<%=pagesize%>&amp;xu=<%=xu%>&amp;paixu=name&amp;keywords=<%=keywords%>" <% if paixu="name" then %>selected<% end if %>>按【姓名】排序</option>
<option value="<%=filename%>?page=1&amp;pagesize=<%=pagesize%>&amp;xu=<%=xu%>&amp;paixu=date&amp;keywords=<%=keywords%>" <% if paixu="date" then %>selected<% end if %>>按【订单时间】排序</option>
...
<td align="center"><a href="http://yxshejitao.taobao.com/tool/ip/?ip=<%=rs("ip")%>" target="_blank"><%=rs("ip")%></a></td>
<td align="center"><%=rs("date")%></td>
<td align="center"><a href="<%=filename%>?action=edit&id=<%=rs("id")%>&page=<%=page%>">查看</a></td>
</tr>
<%if n mod 1=0 then%>
<%end if%>
<%
...
id=request("id")
set rs = server.CreateObject ("adodb.recordset")
sql="select * from yuzhiguo_e_order where id="& id &""
rs.open sql,conn,1,1
%>
...
<tr>
<td colspan="3" align=center class="a1">查看英文订单</td>
</tr>
<tr>
  <td align="center" class="a3">订购产品</td>
  <td colspan="2" class="a3"><input name="title" type="text" id="title" value="<%=rs("title")%>" size="80" maxlength="50" /></td>
</tr>
<tr>
  <td align="center" class="a3">订购数量</td>
  <td colspan="2" class="a3"><input name="quantity" type="text" id="quantity" value="<%=rs("quantity")%>" size="50" maxlength="50" /></td>
</tr>
<tr>
  <td align="center" class="a3">订购说明</td>
  <td colspan="2" class="a3"><textarea name="content" cols="80" rows="5" id="content"><%=rs("content")%></textarea></td>
</tr>
<tr>
  <td align="center" class="a3">订购时间</td>
  <td colspan="2" class="a3"><%=rs("date")%></td>
</tr>
<tr>
  <td align="center" class="a3">IP地址</td>
  <td colspan="2" class="a3"><a href="http://yxshejitao.taobao.com/tool/ip/?ip=<%=rs("ip")%>" target="_blank"><%=rs("ip")%>[点击查询IP位置]</a></td>
</tr>
<tr>
  <td colspan="3" align="center" class="a1">订购人联系信息</td>
  </tr>
<tr>
...
set rs=conn.execute("delete from yuzhiguo_e_order where id In(" & id & ")")
