...
filename="pro_big_class.asp"'页面名称
id=trim(request("id"))
...
<option value="<%=filename%>?page=1&amp;pagesize=<%=pagesize%>&amp;xu=<%=xu%>&amp;paixu=big_id&amp;keywords=<%=keywords%>" <% if paixu="big_id" then %>selected<% end if %>>按【默认】排序</option>
<option value="<%=filename%>?page=1&amp;pagesize=<%=pagesize%>&amp;xu=<%=xu%>&amp;paixu=e_name&amp;keywords=<%=keywords%>" <% if paixu="e_name" then %>selected<% end if %>>按【英文大类名称】排序</option>
...
<tr>
<td colspan="8" align=center class="a1">产品大类管理</td>
</tr>
<tr  style="font-weight:bold;">
<td height="30" align="center" class="a1">选择</td>
<td align="center" class="a1">ID</td>
<td align="center" class="a1">中文大类名称</td>
<td align="center" class="a1">英文大类名称</td>
<td align="center" class="a1">小类数量</td>
...
sql ="select * from yuzhiguo_big_class where  e_name like '%"&keywords&"%' order by "&paixu&" "&xu&""
...
<td align="CENTER"><input type="checkbox" value="<%=rs("big_id")%>" name="id" onClick="unselectall(this.form)" style="border:0;">
<input name="pl_id" type="hidden" id="pl_id" value="<%=rs("big_id")%>"></td>
<td align="center"><%=rs("big_id")%></td>
...
<td align="center"><a href="pro_small_class.asp?big_id=<%=rs("big_id")%>">[ <%=Mydb("Select Count([small_id]) From [yuzhiguo_small_class] where [big_id]="&rs("big_id")&"",1)(0)%> ]</a></td>
<td align="center"><a href="pro_manage.asp?big_id=<%=rs("big_id")%>&paixu=id">[ <%=Mydb("Select Count([id]) From [yuzhiguo_products] where [big_id]="&rs("big_id")&"",1)(0)%> ]</a></td>
<td align="center"><input name="sort" type="text" id="sort" value="<%=rs("sort")%>" size="4" maxlength="10" onKeyDown="myKeyDown()"></td>
<td align="center"><a href="<%=filename%>?action=edit&id=<%=rs("big_id")%>&page=<%=page%>">编辑</a></td>
</tr>
<%if n mod 1=0 then%>
<%end if%>
<%
...
<tr>
<td colspan="4" align=center class="a1">添加产品大类</td>
</tr>
...
if e_name="" or name="" then
Call Alert ("请填写完整再提交！","-1")
end if
If sort="" Then sort=1
set rs = server.CreateObject ("adodb.recordset")
sql="select * from yuzhiguo_big_class "
...
id=request("id")
set rs = server.CreateObject ("adodb.recordset")
sql="select * from yuzhiguo_big_class where big_id="& id &""
rs.open sql,conn,1,1
%>
<table width="99%" border="0"  align=center cellpadding="3" cellspacing="1" bgcolor="#FFFFFF" class="a2">
<form action="<%=filename%>?action=savedit" name="myform" method=post>
<tr>
<td colspan="4" align=center class="a1">修改产品大类</td>
</tr>
...
<tr>
<td align="center" class="a3"><input name="id" type="hidden" id="id" value="<%=rs("big_id")%>">
...
del_id=replace(request.form("id")(i),"'","")
	...
	sql="select * from yuzhiguo_products where big_id="&del_id&""
	rs_del.open sql,conn,1,1
	if rs_del.eof and rs_del.bof then 
	set rs=conn.execute("delete from yuzhiguo_big_class where big_id In(" & del_id & ")")
	'set rs=conn.execute("delete from yuzhiguo_small_class where big_id In(" & del_id & ")")
	set rs=conn.execute("delete from yuzhiguo_products where big_id In(" & del_id & ")")
	else 
...
set rs=conn.Execute("select * from yuzhiguo_big_class where big_id="&pl_id)
conn.Execute("update yuzhiguo_big_class set name='"&name&"',e_name='"&e_name&"',sort="&sort&" where big_id="&pl_id)
next
