...
<title><%=xiaoyuer_webname%> - 网站地图</title>
<meta name="keywords" content="<%=xiaoyuer_keywords%>" />
<meta name="description" content="<%=xiaoyuer_description%>" />
...
<h1><%=xiaoyuer_webname%> - 网站地图</h1>

<%set rs=server.CreateObject("adodb.recordset")
rs.open "select * from  xiaoyuer_menu  where show=false order by sort asc",conn,1,1
...
<%
do while not rs.eof%>
<a href="../c_html_products/<%=rs("html_url_cn")%>"  ><%=rs("cpmc")%> - <%=rs("cpbh")%></a> |
<%rs.movenext
loop
%>
<%
...
<h1>新闻分类</h1>
<%set rs=server.CreateObject("adodb.recordset")
rs.open "select * from  xiaoyuer_news_class",conn,1,1
if rs.eof and rs.bof then
else
%>
<%
do while not rs.eof%>
<div>●<a href="../c_news/?class_id=<%=rs("class_id")%>"  ><%=rs("name")%></a></div>
<%rs.movenext
loop
%>
<%
...
<h1>新闻列表</h1>
<%set rs=server.CreateObject("adodb.recordset")
rs.open "select * from  xiaoyuer_news",conn,1,1
if rs.eof and rs.bof then
else
%>
<%
do while not rs.eof%>
<a href="../c_html_news/<%=rs("html_url")%>"  ><%=rs("name")%></a> |
<%rs.movenext
loop
%>
<%
