 <div class="left">
     <div class="left_title"><h3>Product Search</h3></div>
   <div class="left_c">
...
<form action="/e_products/index.asp" method="post" name="search" id="search">
   ...
   <div class="left_title"><h3>Products Category</h3></div>
   <div class="left_c">
   <%
			...
			%>
      <div class="p_list">
        <div class="list"><a href="<%=webfolder%>e_products/?big_id=<%=big_id%>" title="<%=e_name%>" class="big_class_bg"><strong><% if big_id_now=big_id then%><font color="#FF0000"><%=e_name%></font><%else%><%=e_name%><%end if%></strong></a>
        </div></div>
	  ...
	  <div class="p_list"><div class="list_s"><a href="<%=webfolder%>e_products/?big_id=<%=big_id%>&amp;small_id=<%=small_id%>" title="<%=e_name%>" >└ <% if small_id_now=small_id then%><font color="#FF0000"><%=e_name%></font><%else%><%=e_name%><%end if%></a></div>
	  </div>
	  <%
			rss.movenext
			loop
...
<% if e_name1<>"" then %>
<strong>Name:</strong> <%=e_name1%><br /><% end if %>
<% if tel<>"" then %>
<strong>Tel:</strong> <%=tel%><br /><% end if %>
<% if fax<>"" then %>
<strong>Fax:</strong> <%=fax%><br /><% end if %>
<% if mobile<>"" then %>
<strong>Mobile:</strong> <%=mobile%><br /><% end if %>
<% if email<>"" then %>
<strong>E-mail: </strong><%=email%><br /><% end if %>
<% if e_add<>"" then %>
<strong>Add:</strong> <%=e_add%><br /><% end if %>
<% if msn<>"" then %>
...
<a href="http://wpa.qq.com/msgrd?V=1&Uin=<%=qq%>&Site=<%=e_webname%>&Menu=yes" target="_blank"><img src="<%=webfolder%>images/qq.png" alt="QQ: <%=qq%>" border="0" /></a>
          <% end if %>
<a target="_blank" href="http://amos1.taobao.com/msg.ww?v=2&uid=%E7%88%B1%E9%9D%93%E7%99%BE%E5%88%86%E7%99%BE&s=1"><img border="0" src="<%=webfolder%>images/wangwang.png" alt="点击这里给我发旺旺信息"></a>
<a target="_blank" href="mailto:web@deepne.cn"><img border="0" src="<%=webfolder%>images/email.png" alt="点击这里给我发消息"></a>
